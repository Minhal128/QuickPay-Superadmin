...
type RadiusConfig = {
  ...
  maxProTravelRadiusKm: number;
};

export default function RadiusCoverage() {
  const { data } = useApi<RadiusConfig>("/advance/radius");
  ...
  const minLeadTime = editedMinLeadTime ?? data?.defaultSearchRadiusKm ?? 30;
  const maxBookingWindow = editedMaxBookingWindow ?? data?.maxProTravelRadiusKm ?? 14;

  const patch = async (body: Partial<RadiusConfig>) => {
    try {
      await api("/advance/radius", {
        method: "PATCH",
  ...
  const bookingRules = [
    {
      ...
      onChange: (v: number) => {
        setMinLeadTime(v);
        patch({ defaultSearchRadiusKm: v });
      },
    },
    {
      label: "Max pro travel radius (km)",
